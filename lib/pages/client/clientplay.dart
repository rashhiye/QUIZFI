...
class Question extends StatefulWidget {
  ...
  final int d;
  Question(
      {super.key, required this.strr, required this.size, required this.d});
  @override
...
class _QuestionState extends State<Question> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    getLocalIpAddress();
    //create();
    WidgetsBinding.instance.addObserver(this); //ch
  }

  @override
  void dispose() {
    _timer.cancel();
    WidgetsBinding.instance.removeObserver(this); //ch
    super.dispose();
  }
  ...
  void create() {
    ...
    if (q < n) {
      ...
      _isRunning = false;
      C_send(
          '${_duration.toString()}:${countT.toString()}:${countF.toString()}:${skipped.toString()}:${_elapsedseconds.toStringAsFixed(1)}:$ip');
      Navigator.push(
        context,
        MaterialPageRoute(
